...
const ActiveWebhooks = () => {
   ...
   return (
      <Page title="Webhooks" breadcrumbs={[{ content: 'Home', onAction: () => navigate('/debug') }]}>
         {loading && (
            <Frame>
               <Loading />
            </Frame>
         ...
         >
            <p>
               Webhooks are registered when the app is installed, or when tokens are refetched by going
               through the authentication process. If your Callback URL isn't the same as your current URL
               (happens usually during dev when using ngrok), you need to go through the auth process again.
               Click on `reauth` to fix this.
            </p>
         </LegacyCard>
      </Page>
   )
